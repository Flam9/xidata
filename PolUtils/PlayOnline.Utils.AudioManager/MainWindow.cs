...
namespace PlayOnline.Utils.AudioManager
{
    public partial class MainWindow : Form
    {
        public MainWindow()
        {
            ...
            {
                MessageBox.Show(this, "Managed DirectX is not available; sound playback will be disabled.",
                    "DirectSound Initialization Failed", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void MainWindow_Closed(object sender, System.EventArgs e) { this.StopPlayback(); }
        ...
        private void MainWindow_VisibleChanged(object sender, System.EventArgs e)
        {
            if (this.Visible)
            {
                ...
                {
                case 0:
                    Browser = this.tvMusicBrowser;
                    break;
                case 1:
                    Browser = this.tvSoundBrowser;
                    break;
                }
        ...
        private void tvBrowser_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
        {
            ...
            else
            {
                ...
                if (FI != null && FI.AudioFile != null)
                {
                    ...
                    this.txtFormat.Text = String.Format("{0}-channel {1}-bit {2}Hz {3}", FI.AudioFile.Channels,
                        FI.AudioFile.BitsPerSample, FI.AudioFile.SampleRate, FI.AudioFile.SampleFormat);
        ...
        private void tabBrowsers_SelectedIndexChanged(object sender, System.EventArgs e) { this.MaybeRefreshBrowser(); }
        ...
        private void btnDecode_Click(object sender, System.EventArgs e)
        {
            ...
            if (TN != null)
            {
                ...
                string saveto = $"{path}/{TN.Text}";
                MusicLog($"Extracting: {path}");
        ...
        private void btnPause_Click(object sender, System.EventArgs e) { this.PausePlayback(); }

        private void btnStop_Click(object sender, System.EventArgs e) { this.StopPlayback(); }
		...
		private void DecodeAllMusic_Click(object sender, EventArgs e)
		{
            // tick everything
            CheckAllNodes(this.tvMusicBrowser);
            ...
            // load XML
            MusicLog("Decoding all music...");

            List<TreeNode> nodes = GetCheckedNodes(this.tvMusicBrowser);

            MusicLog($"There are {nodes.Count} songs selected");
            ...
            foreach (TreeNode TN in nodes)
			{
                if (TN != null)
                {
                    ...
                    string saveto = $"{path}/music/{TN.Text}.wav";

                    try
                    {
                        using (WaveWriter WW = new WaveWriter(FI.AudioFile, saveto))
                        {
                            WW.ShowDialog(this);
                        }
                    }
        ...
        public void CheckAllNodes(TreeView treeView)
        {
        ...
        private void GetCheckedNodes(TreeNode node, List<TreeNode> checkedNodes)
        {
            if (node.Checked)
            {
        ...
        private void MusicLog(string text)
		{
            MusicLogBox.AppendText(text + Environment.NewLine);
        }

		private void button1_Click(object sender, EventArgs e)
		{
            // tick everything
            CheckAllNodes(this.tvSoundBrowser);
            ...
            // load XML
            MusicLog("Decoding all soundeffects...");

            List<TreeNode> nodes = GetCheckedNodes(this.tvSoundBrowser);

            MusicLog($"There are {nodes.Count} sounds selected");
            ...
            foreach (TreeNode TN in nodes)
            {
                if (TN != null)
                {
                    ...
                    string saveto = $"{path}/soundeffects/{TN.Text}.wav";
